...
<h2 class="class-name"><img src="{$subdir}images/{if $abstract}{if $access == 'private'}AbstractPrivate{else}Abstract{/if}{else}{if $access == 'private'}Private{/if}{/if}{if $is_interface}Interface{else}Class{/if}_logo.png"
...
<div class="info-box">
	...
	<div class="info-box-body">
		...
		<p class="notes">
			Located in {$source_location} (line <span class="field">{if $class_slink}{$class_slink}{else}{$line_number}{/if}</span>)
		</p>
			...
			<table cellpadding="2" cellspacing="0" class="class-table">
				...
				<tr>
					<td style="padding-right: 2em; white-space: nowrap">
						<img src="{$subdir}images/{if $children[kids].abstract}Abstract{/if}{if $children[kids].access == 'private'}Private{/if}Class.png"
		...
		<div class="info-box-body">
			<div class="const-summary">
				...
				<div class="const-title">
					<img src="{$subdir}images/Constant.png" alt=" " />
					<a href="#const_{$consts[consts].const_name}" title="details" class="const-name">{$consts[consts].const_name}</a> = 					<span class="var-type">{$consts[consts].const_value}</span>
				</div>
				{/section}
			</div>
		</div>
	...
	<div class="info-box">
		...
		<div class="info-box-body">
			<div class="var-summary">
				...
				{if $vars[vars].static}
				<div class="var-title">
					<img src="{$subdir}images/StaticVariable.png" alt=" " />
					static <span class="var-type">{$vars[vars].var_type}</span>
					<a href="#var_{$vars[vars].var_name};" title="details" class="var-name">{$vars[vars].var_name}</a>
				</div>
				{/if}
				{/section}
				{section name=vars loop=$vars}
				{if !$vars[vars].static}
				<div class="var-title">
					<img src="{$subdir}images/{if $vars[vars].access == 'private'}PrivateVariable{else}Variable{/if}.png" alt=" " />
		...
		<div class="info-box-body">			
			<div class="method-summary">
				...
				<div class="method-definition">
					<img src="{$subdir}images/StaticMethod.png" alt=" "/>
					static
					<a href="#method_{$methods[methods].function_name}" title="details" class="method-name">{if $methods[methods].ifunction_call.returnsref}&amp;{/if}{$methods[methods].function_name}</a>
				...
				{if !$methods[methods].static}
				<div class="method-definition">
					<img src="{$subdir}images/{if $methods[methods].ifunction_call.constructor}Constructor{elseif $methods[methods].ifunction_call.destructor}Destructor{elseif $methods[methods].access == 'private'}{if $methods[methods].abstract}Abstract{/if}PrivateMethod{else}{if $methods[methods].abstract}Abstract{/if}Method{/if}.png" alt=" "/>
					<a href="#method_{$methods[methods].function_name}" title="details" class="method-name">{if $methods[methods].ifunction_call.returnsref}&amp;{/if}{$methods[methods].function_name}</a>
					{if count($methods[methods].ifunction_call.params)}
						({section name=params loop=$methods[methods].ifunction_call.params}{if $smarty.section.params.iteration != 1}, {/if}{if $methods[methods].ifunction_call.params[params].hasdefault}[{/if}<span class="var-type">{$methods[methods].ifunction_call.params[params].type}</span>&nbsp;<span class="var-name">{$methods[methods].ifunction_call.params[params].name}</span>{if $methods[methods].ifunction_call.params[params].hasdefault} = <span class="var-default">{$methods[methods].ifunction_call.params[params].default}</span>]{/if}{/section})
					{else}
				...
				{section name=iconsts loop=$iconsts}
					...
					<blockquote>
						{section name=iconsts2 loop=$iconsts[iconsts].iconsts}
							<img src="{$subdir}images/{if $iconsts[iconsts].iconsts[iconsts2].access == 'private'}PrivateVariable{else}Variable{/if}.png" />
