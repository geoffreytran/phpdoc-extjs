...
DocPanel = Ext.extend(Ext.Panel, {
	...
	scrollToSection: function(id){
		var el = Ext.get(id);
		if(el){
			Ext.get(this.el.dom.firstChild.firstChild).scrollTo('t', el.dom.offsetTop, {duration: .5, callback: function() {
				Ext.get(el.dom.nextSibling.nextSibling).highlight();				
			}});
...
strrpos = function(str, needle, start) {
	if (!start) {
		start=0;
	}
	
	var j = start
	var i = str.indexOf(needle, start);;
	
	if (i!=-1) {
		return strrpos(str, needle, i+1);
	}
...
Ext.extend(MainPanel, Ext.TabPanel, {
    ...
    onClick: function(e, target){
        if(target = e.getTarget('a:not(.exi)', 3)){
			...
			var i = strrpos(location.href, '/');
			var wloc = location.href.substring(0, i);
			var internal = target.href.substring(wloc.length, target.href.length);
			var internalSplit = internal.split('#');
			var internalLink, internalNode;
			
			if (internalSplit[0]=='index.html') {
				internal='#'+internalSplit[1];
			}
			else {
				api.root.cascade(function(n){
					var s = n.attributes.href;
					console.log('href', s);
					if (typeof s!='undefined') {
						if(s == internalSplit[0]) {
